...
class Panel extends React.Component {
  ...
  render() {
    ...
    if(this.props.editingEnabled) return (
      <Draggable handle=".panel__bar" cancel=".panel__bar__close"
        ...
        <Resizable height={this.props.panel.height*324 - 24} width={this.props.panel.width*324 - 24} minConstraints={[300, 300]} maxConstraints={[624, 624]}
          ...
          <div className="panel" id={this.props.panel.name} orientation={o}
            ...
            <div className="panel__bar">
              <span className="panel__bar__title">{this.props.panel.name}</span>
              <span className="panel__bar__close" onClick={this.onClose} > X </span>
            </div>
            
            { this.props.panel.name === "Inventory" && <InventoryPanelContent orientation={o}/> }
            { this.props.panel.name === "Shortwave" && <ShortwavePanelContent orientation={o}/> }
            { this.props.panel.name === "Orbit" &&     <OrbitPanelContent     orientation={o}/> }
          </div>
